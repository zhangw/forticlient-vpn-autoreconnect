function debugAndCallConnectVpn() {
    ...
    if (delegate) {
        ...
        try {
            // 根据方法签名创建函数
            var connectVpnFunc = new NativeFunction(
                ptr("0x1000ab7f0"), 
                'void',           // 返回类型
                ['pointer', 'pointer', 'pointer']  // self, selector, parameter
            );
            
            var selector = ObjC.selector("connectVpn:");
            var nsNull = ObjC.classes.NSNull.null();
            
            connectVpnFunc(delegate.handle, selector, nsNull.handle);
            console.log("✅ NativeFunction connectVpn 调用成功");
            return;
        } catch (e) {
